...
if_else_tuple(switch::Val{false}, old, new) = old
if_else_tuple(switch::Val{true}, old, new) = new

get_index(into::Tuple{}, index) = ()
get_index(into, index) = begin
    next = get_index(tail(into), tail(index))
    if_else_tuple(first(index), next, (first(into), next...))
end
...
set_index(old, new, switch) =  begin
    ...
    if_else_tuple(first_switch, first(old), first(new)),
    set_index(
        tail(old),
        if_else_tuple(first_switch, new, tail(new)),
        tail(switch))...
end
...
fill_index(old, new, switch) =  begin
    first_switch = first(switch)
    if_else_tuple(first_switch, first(old), new),
    fill_index(
        tail(old),
        new,
        tail(switch))...
